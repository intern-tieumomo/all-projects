 ...
 */
if(!class_exists('EldritchEdgeTopNavigationWalker')) {
	class EldritchEdgeTopNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl(&$output, $depth = 0, $args = array()) {
			...
			if($this->background_image !== '') {
				...
				$background_image_class = ' edgt-dropdown-with-background-image';
			}

			$indent = str_repeat("\t", $depth);
			if($depth == 0) {
				$out_div = '<div class="second '.$background_image_class.'" '.eldritch_edge_get_inline_style($menu_image_background).'><div class="inner">';
			} else {
				$out_div = '';
			}
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
			...
			if(!empty($item->icon)) {
				$class_names .= ' edgt-menu-item-with-icon';
			}

			if(!empty($item->description)) {
				$class_names .= ' edgt-menu-item-with-description';
			}

			$active = "";
			// depth dependent classes
			if($item->anchor == "" && (($item->current && $depth == 0) || ($item->current_item_ancestor && $depth == 0))):
				$active = 'edgt-active-item';
			endif;
			...
			if($item->hide == '') {
				...
				if($item->featured_icon !== '') {
					$featured_icon .= '<span class="edgt-menu-featured-icon '.$item->featured_icon.'" aria-hidden="true"></span>';
				}

				$item_output .= $featured_icon;

				if(!empty($item->description)) {
					$item_output .= '<span class="edgt-menu-item-description">'.$item->description.'</span>';
				}

				$item_output .= '</span>'; //close span.item_text

//                if($args->has_children && $depth == 0) {
//                    $item_output .= '<span class="edgt-menu-arrow arrow_carrot-down"></span>';
//                }
				...
				if($args->has_children && $angle_icon == "yes") {
                    $item_output .= '<span class="q_menu_arrow"></span>';
				}
