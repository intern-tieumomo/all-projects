...
class HeaderMinimal extends HeaderType {
	...
	public function __construct() {
		...
		if (!is_admin()) {
            ...
            $menuAreaHeight = eldritch_edge_filter_px(eldritch_edge_get_meta_field_intersect('menu_area_height_header_minimal', $id));
	...
	public function headerPerPageStyles($style) {
		...
		$disable_shadow = eldritch_edge_get_meta_field_intersect('menu_area_shadow_header_minimal', $id) == 'no';
		$disable_grid_shadow = eldritch_edge_get_meta_field_intersect('menu_area_in_grid_shadow_header_minimal', $id) == 'no';
        $current_style = '';

		$main_menu_selector = array($class_prefix . '.edgt-header-minimal .edgt-menu-area');
		$main_menu_grid_selector = array($class_prefix . '.edgt-header-minimal .edgt-page-header .edgt-menu-area .edgt-grid .edgt-vertical-align-containers');

		/* header style - start */

		$menu_area_background_color = get_post_meta($id, 'edgt_menu_area_background_color_header_minimal_meta', true);
		$menu_area_background_transparency = get_post_meta($id, 'edgt_menu_area_background_transparency_header_minimal_meta', true);
        ...
        $menu_area_height = get_post_meta($id, 'edgt_menu_area_height_header_minimal_meta', true);

        if ($menu_area_height !== '') {
            $max_height = intval(eldritch_edge_filter_px($menu_area_height) * 0.9).'px';
            $current_style .= eldritch_edge_dynamic_css('.edgt-header-minimal .edgt-page-header .edgt-logo-wrapper a', array('max-height' => $max_height));

            $main_menu_style['height'] = eldritch_edge_filter_px($menu_area_height).'px';
        }

        $menu_area_disable_background_image = get_post_meta($id, 'edgt_menu_area_disable_background_image_header_minimal_meta', true);
        if($menu_area_disable_background_image !== 'yes') {
            $menu_area_background_image = eldritch_edge_get_meta_field_intersect('menu_area_background_image_header_minimal', $id);
        ...
        $menu_area_border = eldritch_edge_get_meta_field_intersect('menu_area_border_header_minimal', $id);

        if ($menu_area_border == 'yes') {

            $menu_area_border_color = eldritch_edge_get_meta_field_intersect('menu_area_border_color_header_minimal', $id);
		...
		$menu_area_grid_background_color = get_post_meta($id, 'edgt_menu_area_grid_background_color_header_minimal_meta', true);
		$menu_area_grid_background_transparency = get_post_meta($id, 'edgt_menu_area_grid_background_transparency_header_minimal_meta', true);
	...
	public function loadTemplate($parameters = array()) {
		...
		$parameters['menu_area_in_grid'] = eldritch_edge_get_meta_field_intersect('menu_area_in_grid_header_minimal', $id) == 'yes' ? true : false;

		$parameters = apply_filters('eldritch_edge_header_minimal_parameters', $parameters);
	...
	public function calculateHeightOfTransparency() {
		...
		if (get_post_meta($id, 'edgt_menu_area_background_color_header_minimal_meta', true) !== '') {
			$menuAreaTransparent = get_post_meta($id, 'edgt_menu_area_background_color_header_minimal_meta', true) !== '' &&
				get_post_meta($id, 'edgt_menu_area_background_transparency_header_minimal_meta', true) !== '1';
		} elseif (eldritch_edge_options()->getOptionValue('menu_area_background_color_header_minimal') == '') {
			$menuAreaTransparent = eldritch_edge_options()->getOptionValue('menu_area_grid_background_color_header_minimal') !== '' &&
				eldritch_edge_options()->getOptionValue('menu_area_grid_background_transparency_header_minimal') !== '1';
		} else {
			$menuAreaTransparent = eldritch_edge_options()->getOptionValue('menu_area_background_color_header_minimal') !== '' &&
				eldritch_edge_options()->getOptionValue('menu_area_background_transparency_header_minimal') !== '1';
		}
	...
	public function calculateHeightOfCompleteTransparency() {
		...
		if (get_post_meta($id, 'edgt_menu_area_background_color_header_minimal_meta', true) !== '') {
			$menuAreaTransparent = get_post_meta($id, 'edgt_menu_area_background_color_header_minimal_meta', true) !== '' &&
				get_post_meta($id, 'edgt_menu_area_background_transparency_header_minimal_meta', true) === '0';
		} elseif (eldritch_edge_options()->getOptionValue('menu_area_background_color_header_minimal') == '') {
			$menuAreaTransparent = eldritch_edge_options()->getOptionValue('menu_area_grid_background_color_header_minimal') !== '' &&
				eldritch_edge_options()->getOptionValue('menu_area_grid_background_transparency_header_minimal') === '0';
		} else {
			$menuAreaTransparent = eldritch_edge_options()->getOptionValue('menu_area_background_color_header_minimal') !== '' &&
				eldritch_edge_options()->getOptionValue('menu_area_background_transparency_header_minimal') === '0';
		}
