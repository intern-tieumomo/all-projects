 ...
 *
 * Class EldritchEdgeWidget
 */
abstract class EldritchEdgeWidget extends WP_Widget {
	/**
				...
				switch($param['type']) {
					case 'textfield':
						...
						<p>
							...
							<?php if(!empty($param['description'])) : ?>
								<span class="edgt-field-description"><?php echo esc_html($param['description']); ?></span>
							<?php endif; ?>
						</p>
						<?php
						break;
                    ...
                    case 'textarea': ?>
                        <p>
                            <label for="<?php echo esc_attr($this->get_field_id($param['name'])); ?>"><?php echo
                                esc_html($param['title']); ?>:</label>
                            <textarea rows="8" class="widefat" id="<?php echo esc_attr($this->get_field_id($param['name'])); ?>" name="<?php echo esc_attr($this->get_field_name($param['name'])); ?>"><?php echo eldritch_edge_get_module_part(${$param['name']}); ?></textarea>
                            <?php if(!empty($param['description'])) : ?>
                                <span class="edgt-field-description"><?php echo esc_html($param['description']); ?></span>
                            <?php endif; ?>
                        </p>
